		...
		$.ajax({
			...
			success: function(data){
				...
				if($scope.user.role == '0'){
					...
					window.location.href = "http://localhost:8000/#/teacher";
				}else if($scope.user.role == '2'){
					window.location.href = "http://localhost:8000/#/boss";
				}
			},
			error: function(error){
				console.log(error);
						...
						.success(function(data){
							if(data.success == "true"){
								...
								if(data.user.userRole == "0"){
									...
									if($scope.user.QQ==null || $scope.user.tel==null || $scope.user.email==null){
										$location.path("/tpersonalInfo");
									}else{
										$location.path("/teacher");
									}
								}else if(data.user.userRole == "2"){
									if($scope.user.QQ==null || $scope.user.tel==null || $scope.user.email==null){
										$location.path("/bpersonalInfo");
									}else{
										$location.path("/boss");
									}	
								}
...
mainApp.controller("announceCtrl", ['$scope', '$http', '$filter', 'announceService', function($scope, $http, $filter, announceService){
...
/*
	三级用户控制器
*/
mainApp.controller("sActivityCtrl", ['$scope', '$filter', '$http', '$location', '$routeParams', 'activityService', function($scope, $filter, $http, $location, $routeParams, activityService){
    ...
    $scope.chooseActivity = function(type){
    	if(type == "new"){
    		...
    		url = 'http://www.ly723.site/StudentManager2/student/Signs.action';
    		$scope.sActivityPage.onChange();
    	}
	}
...
/*
	二级用户控制器
*/
mainApp.controller("tReportCtrl", ['$scope', '$http', function($scope, $http){
...
mainApp.controller("tTaskCtrl", ['$scope', '$http', '$location', 'taskService', 'activityService', function($scope, $http, $location, taskService, activityService){
...
mainApp.controller("tActivityCtrl", ['$scope', '$http', '$filter', '$location', '$routeParams', 'activityService', function($scope, $http, $filter, $location, $routeParams, activityService){
	$scope.activities = activityService.get();
	...
	$scope.activity = null;
	$scope.$on("$routeChangeSuccess", function(){
		if($location.path().indexOf("/dealAct/") == 0){
			var id = $routeParams["id"];
			for(var i = 0; i < $scope.activities.length; i++){
				if($scope.activities[i].acId == id){
					$scope.activity = $scope.activities[i];
		...
		if($location.path().indexOf("/applyDetail/") == 0){
			var id = $routeParams["id"];
			for(var i = 0; i < $scope.activities.length; i++){
				if($scope.activities[i].acId == id){
					$scope.activity = $scope.activities[i];
...
mainApp.controller("tInformCtrl", ['$scope', '$http', 'informService', function($scope, $http, informService){
...
mainApp.controller("tMessageCtrl", ['$scope', '$http', '$filter', function($scope, $http, $filter){
	...
	$scope.sendMessage = function(){
		...
		for (var i = 0; i < receiver.length; i++) {
			var myDate = new Date();
			var currentTime = $filter("date")(myDate.getTime(), "yyyy-MM-dd HH:mm");
			var data = '{"notSenId": "' + $scope.user.userId + '", "notRecId": "' + 
				receiver[i] + '", "notDet": "' + $("#message").find("textarea").val() + '", "notSenDate": "' + currentTime + '" }';
			$.ajax({
				type: 'post',
				url: 'http://www.ly723.site/StudentManager2/insertMessage.action',
				data: data,
				contentType: "application/json;charset=utf-8",
				success: function(data){
					if(data == 1){
					...
					window.location.href = "http://localhost:8000/#/tpersonalInfo";
				}
			});
		};
	}
...
/*
	一级用户控制器
*/
mainApp.controller("bReportCtrl", ['$scope', '$http', function($scope, $http){
...
mainApp.controller("bActivityCtrl", ['$scope', '$http', '$location', '$routeParams', 'activityService', function($scope, $http, $location, $routeParams, activityService){
	$scope.activities = activityService.get();
    ...
    $scope.$on("$routeChangeSuccess", function(){
		if($location.path().indexOf("/alterAct/") == 0){
			var id = $routeParams["id"];
			for (var i = 0; i < $scope.activities.length; i++) {
				if($scope.activities[i].acId == id){
					$scope.activity = $scope.activities[i];
    ...
    $scope.addActivity = function(){
		$("#activityForm").prepend("<input type='hidden' id='tasAcademy' name='tasAcademy' value = '" + $scope.toWho + "' />");
...
mainApp.controller("bTaskCtrl", ['$scope', '$http', '$filter', '$location', '$routeParams', 'taskService', function($scope, $http, $filter, $location, $routeParams, taskService){
	...
	$scope.$on("$routeChangeSuccess", function(){
		...
		if($location.path().indexOf("/altTask/") == 0){
			var id = $routeParams["id"];
	...
	$scope.createTask = function(){
		...
		var url = '';
		if($scope.toWho == '' || $scope.toWho == null){
			alert('请选择分配给谁!');
			return false;
		}
		if($scope.choose){
			formData.append("tasDueId", $scope.toWho.userId);
			url = 'http://www.ly723.site/StudentManager2/boss/insertTask.action';
		}else{
			formData.append("tasAcademy", $scope.toWho);
			url = 'http://www.ly723.site/StudentManager2/boss/insertTaskByAcdemy.action'
		}
     	$.ajax({ 
         	async: false,
         	url : url,  
	...
	// $scope.selectMyTask = function(){
	// 	var url = 'http://www.ly723.site/StudentManager2/boss/selectTaskByTasCreId.action';
	// 	var config = {
 //           	headers: {
 //           		'Content-Type':'application/json; charset=utf-8'
 //           	}	
 //       	};
 //     	var data = JSON.stringify({"tasCreId": $scope.user.userId});
...
mainApp.controller("bMessageCtrl", ['$scope', '$http', '$filter', function($scope, $http, $filter){
		...
		for (var i = 0; i < receiver.length; i++) {
			...
			$.ajax({
				...
				success: function(data){
					...
					window.location.href = "http://localhost:8000/#/bpersonalInfo";
				}
			});
		};
	}
